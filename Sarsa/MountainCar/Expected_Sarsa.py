# -*- coding: utf-8 -*-
"""
Created on Tue Feb  7 15:52:33 2023
...
for episode in range(EPISODES) :
    ...
    while not done : 
        ...
        if not done : 
            q_max = np.max(q_table[new_discrete_state])
            nb_greedy_actions = 0
            for i in range(env.action_space.n):
                if q_table[new_discrete_state][i] == q_max:
                    nb_greedy_actions += 1
         
            non_greedy_action_prob = epsilon/env.action_space.n
            greedy_action_prob = ((1 - epsilon)/nb_greedy_actions) + non_greedy_action_prob
     
            expected_q = 0
            for i in range(env.action_space.n):
                if q_table[new_discrete_state][i] == q_max:
                    expected_q += q_table[new_discrete_state][i] * greedy_action_prob
                else:
                    expected_q += q_table[new_discrete_state][i] * non_greedy_action_prob
     
            q_table[discrete_state + (action,)] = (1-lr)*q_table[discrete_state + (action,)] + lr * (reward + gamma * expected_q)
...
with open('reward_expected_sarsa.pickle', 'wb') as file:
